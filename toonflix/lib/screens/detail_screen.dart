...
class _DetailScreenState extends State<DetailScreen> {
  ...
  onHeartTap() async {
    final likedToons = prefs.getStringList('likedToons') ?? [];
    final updatedToons = List<String>.from(likedToons);

    if (isLiked) {
      updatedToons.remove(widget.id);
    } else {
      updatedToons.add(widget.id);
    }

    await prefs.setStringList('likedToons', updatedToons); // 저장소에 저장하고
    setState(() {
      isLiked = !isLiked;
    });
  }
          ...
          child: Column(
            children: [
              Row(
                ...
                children: [ImageCard(thumb: widget.thumb, id: widget.id)],
              ),
